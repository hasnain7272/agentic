...
export function ChatComposer({
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-slate-800 bg-slate-950 p-2 transition focus-within:border-emerald-500/60">
      <div className="flex items-center gap-2 border-b border-slate-800 px-2 pb-2">
        <div className="relative min-w-0 flex-1 sm:flex-none">
          <Cpu className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-emerald-300" />
          <select
            value={activeModel?.id || ''}
            onChange={(event) => onModelSelect(event.target.value)}
            disabled={streaming || modelOptions.length === 0}
            className="h-8 w-full appearance-none rounded-md border border-slate-700 bg-slate-900 pl-8 pr-8 text-xs font-semibold text-slate-100 outline-none transition hover:border-emerald-500/40 disabled:opacity-50 sm:w-auto"
        ...
        <div className="hidden min-w-0 flex-1 items-center gap-2 overflow-hidden text-[10px] text-slate-500 sm:flex">
          ...
          {activeModel?.base_url && <span className="shrink-0 rounded-full bg-emerald-500/10 px-2 py-0.5 text-emerald-300">OpenAI-compat</span>}
        </div>
        <button type="button" onClick={onToggleShadow} title="Shadow mode: simulate restricted actions" className={`flex h-8 shrink-0 items-center gap-1.5 rounded-md px-2.5 text-[11px] font-semibold transition ${shadowMode ? 'bg-amber-500/20 text-amber-300 ring-1 ring-amber-500/50' : 'text-slate-500 hover:bg-slate-800 hover:text-slate-300'}`}>
      ...
      <div className="flex items-end gap-2 px-2 pt-2">
        ...
        <label className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-md text-slate-400 transition hover:bg-slate-800 hover:text-slate-200" title="Attach file">
          <input type="file" className="hidden" onChange={onUpload} />
          <Paperclip className="h-4 w-4" />
        </label>
        <button type="submit" disabled={streaming || !input.trim() || modelOptions.length === 0} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-emerald-600 text-white transition hover:bg-emerald-500 disabled:opacity-30">
